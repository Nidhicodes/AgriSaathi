...
const EquipmentCard = ({ equipment }) => {
  ...
  // Defensive: If no equipment passed, show fallback UI or null
  if (!equipment) {
    return <div>{t('agri_share.card.invalid_equipment')}</div>;
  }

  // Defensive: ensure image_urls is an array
  const images = Array.isArray(equipment.image_urls) ? equipment.image_urls : [];
    ...
    <>
      <div className="bg-white border rounded-lg shadow-md overflow-hidden transition-transform transform hover:-translate-y-1">
        <div className="relative">
          <img
            src={primaryImage}
            alt={equipment.name || t('agri_share.card.unnamed')}
          ...
          {images.length > 1 && (
            <div className="absolute top-2 right-2 bg-black bg-opacity-50 text-white text-xs font-bold py-1 px-2 rounded-full">
              {images.length} {t('agri_share.card.photos')}
            </div>
          )}
        </div>
        ...
        <div className="p-4">
          <h3 className="text-xl font-bold text-gray-800 truncate">
            {equipment.name || t('agri_share.card.unnamed')}
          </h3>
          ...
          <div className="space-y-2">
            ...
            {equipment.contact_whatsapp && (
              <div>
                ...
                <a
                  href={`https://wa.me/${equipment.contact_whatsapp}`}
                  target="_blank"
      ...
      {isModalOpen && <ImageGalleryModal images={images} onClose={() => setIsModalOpen(false)} />}
    </>
  );
};
